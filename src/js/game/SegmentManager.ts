  ...
  update() {
    ...
    if (relativePosition > this.currentSegment.length) {
      if (this.engine.joyrideMode) {
        const nextSegment = this.startSegment
      ...
      const nextSegment = this.sections[this.count % this.sections.length]
