...
class PixiDust extends PIXI.Container {
  ...
  update() {
    for (let i = 0; i < this.particals.length; i++) {
      ...
      const scaleRatio = this.focalLength / (this.focalLength + partical.z)

      partical.scale.x = partical.scale.y = scaleRatio // * 0.5;

      partical.home.x += partical.speed?.x || 0
      partical.position.y += partical.speed?.y || 0
